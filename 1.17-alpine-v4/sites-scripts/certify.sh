...
echo "Creating SSL certs for ${domain}..."

# Copy nginx config template
cp /sites-scripts/template.conf /etc/nginx/conf.d/${domain}.conf

# Replace $DOMAIN placeholder in default.conf with real domain
replace_domain --domain ${domain} --conf-file /etc/nginx/conf.d/${domain}.conf
